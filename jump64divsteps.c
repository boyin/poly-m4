...
void gf_polymul_32x32_2x2_x2p2 (int *V,int *M,int *fh,int *gh){
  ...
  for (X=V, Y=B64_1, Z=B64_2, W=M, i=16; i>0; i--) {// x(u fh+v gh)+f1
    //V[i] = barrett_16x2i(__SADD16(__SADD16(M[i],B64_1[i]),B64_2[i]));
    *(X++) = barrett_16x2i(__SADD16(__SADD16(*(W++),*(Y++)),*(Z++)));
  }  
  for (i=16; i>0; i--) {  
    //V[i+16] = barrett_16x2i(__SADD16(B16_1[i+64],B16_2[i+64]));
  ...
  for (Y=BB64_1, i=16; i>0; i--) {	// x(r fh+s gh) + g1
    //V[i+32] = barrett_16x2i(__SADD16(__SADD16((BB64_1[i],V[i+32])),M[i+16]);
    T = barrett_16x2i(__SADD16(__SADD16(*(W++),*(Y++)),*X)); *(X++) = T;
  } 
  for (i=16; i>0; i--) {  
    //V[i+48] = barrett_16x2i(__SADD16(BB64_1[i+16],V[i+48]));
